...
function wpec_header(){
	...
	echo PHP_EOL.'<link rel="stylesheet" type="text/css" media="screen" href="'.get_settings('siteurl').'/wp-content/plugins/wp-easy-contents/wp_easy_contents.css" />'.PHP_EOL;
}
	...
	if(is_single()){	
		if(!preg_match('|<!--contents-->|', $content)) {
			...
			foreach ($matches as $i=>$val) {
				$table .= "\t".'<li><a href="'.$_SERVER['REQUEST_URI'].'#'.nice_url($val[1]).'" title="'.$val[1].'">'.wp_break_maxchar($val[1]).'</a></li>'.PHP_EOL;
			}
			foreach($add as $val){
				if(!empty($val)){
					$table .= "\t".'<li><a href="'.$_SERVER['REQUEST_URI'].'#'.nice_url($val).'" title="'.$val.'">'.wp_break_maxchar($val).'</a></li>'.PHP_EOL;
				}
			}
...
function wpeasy_contents_subpanel(){
	...
	<div class="wrap"> 
        ...
        <form name="form1" method="post" action="<?php echo $location ?>&amp;updated=true">
            ...
            <td><input name="wpec_float" type="text" id="wpec_float" value="<?php echo $wpec_float; ?>" size="40" class="regular-text code" /></td>
